 ...
 */
define(['N/ui/serverWidget', 'N/search', 'N/record', 'N/runtime', 'N/redirect', 'N/url', 'N/log', 'N/https'], (serverWidget, search, record, runtime, redirect, url, log, https) => 
{
    ...
    function saveChatMessage(sessionId, role, messageText) {
        ...
        // Optional: set sequence number
        msgRec.setValue({ fieldId: 'custrecord_ai_msg_sequence', value: new Date().getTime() });
        msgRec.save();
    }

    // ... (deleteChat, renameChat, getSessions, getChatMessages remain same as before)
    ...
    function getFullHtml(sessions, currentSessionId, messages) {
        // ... (same as previous version - sidebar + messages rendering)
        ...
        let messagesHtml = messages.length === 0 
            ...
            : messages.map(msg => `
                <div class="${msg.role === 'user' ? 'user-msg' : 'assistant-msg'}">
                    <div class="msg">${msg.message.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>
                </div>`).join('');
...
<head>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .msg{display:inline-block;padding:12px 16px;border-radius:12px;max-width:70%;}
...
<body>
    ...
    <script>
        ...
        // ... toggleMenu, renameChat, deleteChat (same as before)
